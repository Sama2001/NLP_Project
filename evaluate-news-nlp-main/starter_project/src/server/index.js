...
async function scrapeTextFromURL(url) {
    try {
        ...
        const articleContent = $('.mw-parser-output');

        // Remove unwanted elements (e.g., tables, infoboxes, references, CSS styles)
        articleContent.find('table, .infobox, .reference, .hatnote, .mw-editsection, .noprint, .sidebar').remove();

        // Extract text from all paragraphs
        let articleText = '';
        articleContent.find('p').each((index, element) => {
            articleText += $(element).text().trim() + ' ';
        });

        // Check if text content exists
        if (!articleText) {
            console.error('No article content found at the provided URL');
            return null;
        }

        // Extract and return the first 200 characters of the text
        const trimmedText = articleText.slice(0, 200);
...
app.post('/analyze-url', async (req, res) => {
    ...
    try {
        ...
        return res.status(500).json({ error: 'Failed to analyze the URL', details: error.message });
    }
});
// Default route
app.get('/', (req, res) => {
